...
import { BarsStageMerge } from "./components/BarsStageMerge";
import { CodePanelMerge } from "./components/CodePanelMerge";
import { StepLabelMerge } from "./components/StepLabelMerge";
import { VfxLayerMerge } from "./components/VfxLayerMerge";
import { SfxTrackMerge } from "./components/SfxTrackMerge";
import { buildSteps, SortStep } from "./lib/sort-merge";
...
export const MainVideoMerge: React.FC = () => {
  ...
  const introStep: SortStep = {
    ...
    j: 0,
    bufPos: 0,
    buffer: Array(N).fill(null),
    source: Array(N).fill(null),
    active: [],
    locked: [],
    rangeStack: [],
    pass: 0,
  ...
  const lastMergeIdx = React.useMemo(() => {
    for (let i = steps.length - 1; i >= 0; i--)
      if (steps[i].kind === "mergeDone") return i;
    return -1;
  }, [steps]);
  const lastMergeFrame =
    lastMergeIdx >= 0 ? SORT_START + clock.starts[lastMergeIdx] : OUTRO_START;
  const finalFlashActive = frame >= lastMergeFrame && frame <= lastMergeFrame + 26;
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div
              style={{
                ...
                boxShadow:
                  "0 12px 30px rgba(122,184,255,0.45), inset 0 2px 0 rgba(255,255,255,0.6)",
                display: "flex",
                alignItems: "center",
